    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="box box-info">
                ...
                <div class="box-body">
                    <a href="{{ route('crear_seccion') }}" class="btn btn-primary">Crear sección</a>
                </div>
            </div>
        </div>
                    ...
                    <div class="box-body">
                        <div class="col-md-3">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" name="id" placeholder="ID#" value="{{ $listado->old('id') }}">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="input-group">
                                <span class="input-group-addon"><i class="glyphicon glyphicon-search"></i></span>
                                <input type="text" class="form-control" name="query" placeholder="Buscar sección..." value="{{ $listado->old('query') }}">
                            </div>
                        </div>
                    </div>
            ...
            <table id="tabla-ordenable" class="table table-striped table-hover">
                ...
                <tbody>
                    @forelse($secciones as $seccion)
                        <tr>
                            <td class="hidden">{{ $seccion->orden }}</td>
                            <td>{{ $seccion->id }}</td>
                            <td>{{ $seccion->nombre }}</td>
                            <td class="text-right">
                                {!! accion_visibilidad($seccion->visible, route('visibilidad_seccion',compact('seccion'))) !!}
                                <a href="{{ route('editar_seccion', compact('seccion')) }}" role="button" class="btn btn-warning btn-circle"><i class="glyphicon glyphicon-edit"></i></a>
                                <a href="{{ route('eliminar_seccion', compact('seccion')) }}" role="button" class="btn btn-danger btn-circle axys-confirmar-eliminar"><i class="glyphicon glyphicon-remove"></i></a>
                            </td>
                            <td class="hidden">{{ $seccion->id }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3">No se encontraron secciones.</td>
                        </tr>
                    @endforelse
                    <?php $seccion = null; ?>
                </tbody>
