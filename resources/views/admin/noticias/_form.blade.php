...
<div class="col-md-4 form-group{{ has_error($errors,'seccion') }}">
    ...
    <select name="id_seccion" class="form-control">
        @foreach($secciones as $seccion)
            <option value="{{ $seccion->id }}"{{ selected($seccion->id==old('id_seccion', $noticia->id_seccion)) }}>{{ $seccion->nombre }}</option>
        @endforeach
    </select>
</div>
<div class="col-md-4 form-group{{ has_error($errors,'region') }}">
    ...
    <select name="id_region" class="form-control">
        @foreach($regiones as $region)
            <option value="{{ $region->id }}"{{ selected($region->id==old('id_region', $noticia->id_region)) }}>{{ $region->nombre }}</option>
        @endforeach
    </select>
</div>
<div class="col-md-2 form-group{{ has_error($errors,'con_video') }}">
    ...
    <div>
        <input type="checkbox" data-toggle="toggle" data-on="Sí" data-off="No" name="con_video" value="1" {{ old('con_video', $noticia->con_video) ? 'checked' : '' }}>
    </div>
</div>
<?php  /*
<div class="col-md-2 form-group{{ has_error($errors,'id_noticia_update') }}">
    <label>ID nuevo</label>
    <input type="text" class="form-control" name="id_noticia_update" value="{{ old('id_noticia_update',$noticia->id) }}">
</div>
*/ ?>
    ...
    @if($noticia->tiene('thumbnail'))
        <div style="position:relative;">
            <div style="position:absolute; left:-14px; top:4px;">
                <a href="{{ route('eliminar_archivo_noticia', ['noticia' => $noticia, 'campo' => 'thumbnail']) }}" class="btn btn-circle btn-sm btn-danger" title="Eliminar"><span class="glyphicon glyphicon-remove"></span></a>
            </div>
...
<div class="col-md-4 form-group{{ has_error($errors,'thumbnail_celular') }}">
    ...
    @if($noticia->tiene('thumbnail_celular'))
        <div style="position:relative;">
            <div style="position:absolute; left:-14px; top:4px;">
                <a href="{{ route('eliminar_archivo_noticia', ['noticia' => $noticia, 'campo' => 'thumbnail_celular']) }}" class="btn btn-circle btn-sm btn-danger" title="Eliminar"><span class="glyphicon glyphicon-remove"></span></a>
            </div>
            <a href="{{ $noticia->url('thumbnail_celular') }}" data-lity><img src="{{ $noticia->url('thumbnail_celular') }}"></a>
        </div>
    @else
