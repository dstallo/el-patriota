    ...
    <div class="row">
        ...
        <div class="col-md-4">
            <div class="box box-info">
                ...
                <div class="box-body">
                    <a href="{{ route('crear_video') }}" class="btn btn-primary">Crear video</a>
                </div>
            </div>
        </div>
                    ...
                    <div class="box-body">
                        ...
                        <div class="col-md-6">
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" name="buscando" placeholder="Buscar video..."
            ...
            <table id="tabla-ordenable" class="table table-striped table-hover">
                ...
                <tbody>
                    @forelse($videos as $video)
                        <tr>
                            <td class="hidden">{{ $video->orden }}</td>
                            <td>{{ $video->id }}</td>
                            <td>{{ $video->nombre }}</td>
                            <td>{{ $video->destacado ? 'Sí' : 'No' }}</td>
                            <td class="text-right">
                                {!! accion_visibilidad($video->visible, route('visibilidad_video', compact('video'))) !!}
                                <a href="{{ route('editar_video', compact('video')) }}" role="button"
                                    class="btn btn-warning btn-circle"><i class="glyphicon glyphicon-edit"></i></a>
                                <a href="{{ route('eliminar_video', compact('video')) }}" role="button"
                    ...
                    @empty
                        <tr>
                            <td colspan="99">No se encontraron videos.</td>
                        </tr>
                    @endforelse
                    <?php $video = null; ?>
                </tbody>
                    ...
                    $.ajax({
                        ...
                        success: function(ret) {
                            if (ret.ok) {
                                ...
                                sweetAlert('Error',
                                    'Hubo un error al actualizar el orden de los elementos, por favor intentá nuevamente.',
                                    'error');
                            }
                        },
                        error: function() {
                            sweetAlert('Error',
                                'Hubo un error al actualizar el orden de los elementos, por favor recargá la página e intentá nuevamente.',
                                'error');
                        }
                    });
                }
